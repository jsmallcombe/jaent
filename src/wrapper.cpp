...
//        h->SetDirectory(0);          for the current histogram h
	...
	// Set the target
	//
	double density_targ=21.46;//adjusted for enrichment g/cm2	
	double density_back=1;//g/cm2
	target tharget(78,196,2000);//1.5 mg/cm2 Pt196 + 2 um Pb 207
	experiment.set_targ(tharget);

	//
	// Set the beam
	//
	double ebeam=36.0; //MeV
	experiment.set_beam(2,4,ebeam);	
	...
	double excited_Z=-1;
	double excited_A=-1;
	double initial_state_MeV = 0.8;
	double transition_keV = 800;
	bool excited_beam_like=false;
	double halflife=0.001;//in ns
